...
export async function ensureSchema() {
  if (!schemaPromise) {
    ...
    schemaPromise = (async () => {
      ...
      await db`ALTER TABLE posts ADD COLUMN IF NOT EXISTS author_image_url text`;
      await db`
      ...
      await db`ALTER TABLE post_comments ADD COLUMN IF NOT EXISTS author_image_url text`;
      await db`
      ...
      await db`ALTER TABLE group_chat_messages ADD COLUMN IF NOT EXISTS author_image_url text`;
      await db`
      ...
      `;
      await db`CREATE INDEX IF NOT EXISTS user_profiles_updated_at_idx ON user_profiles (updated_at DESC)`;
      await db`
      ...
      `;
      await db`ALTER TABLE user_bans ADD COLUMN IF NOT EXISTS permanent boolean NOT NULL DEFAULT true`;
      await db`CREATE INDEX IF NOT EXISTS user_bans_created_at_idx ON user_bans (created_at DESC)`;
    })();
  }
